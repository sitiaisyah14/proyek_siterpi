...
<head>
    ...
    <title>Laporan Data Stok Pakan</title>
...
<body>
    <table width="100%">
        ...
        <br>
            <h6 style="text-align: center;" > LAPORAN DATA STOK PAKAN</h6>
            <table style="border: none" class="fs ml-5">
                ...
                <tr>
                    <td class="font-weight-bold">Nama Pakan</td>
                    <td>:</td>
                    <td> {{$feed->nama_pakan ?? ''}}</td>
                </tr>
            </table>
    ...
    <table class="table table-bordered">
        <thead>
            <tr>
                ...
                <th class="text-center fs">Nama Pakan</th>
        ...
        <tbody>
            @foreach ($feedhis as $data)
                <tr>
                    ...
                    <td class="fs text-center">{{ $data->user_name }}</td>
                    <td class="fs text-center">{{ $data->feed_name }}</td>
                    <td class="fs text-center">{!! date('d F Y', strtotime($data->tanggal))!!}</td>
                    <td class="fs text-center">{{ $data->masuk }} kg</td>
                    <td class="fs text-center">{{ $data->keluar}} kg</td>
                </tr>
            @endforeach
        </tbody>
    </table>
