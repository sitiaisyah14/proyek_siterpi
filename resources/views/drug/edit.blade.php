...
@section('content')
    ...
    <div class="card">
        <div class="card-body">
            ...
            <!-- Multi Columns Form -->
            <form class="row g-3" method="POST" action="{{ route('drug.update',$drug->id)}}">
                @method('PUT')
                @csrf
                <div class="col-4">
                    <label for="inputAddress" class="form-label">Nama Obat</label>
                    <input type="text" class="form-control" name="nama_obat" id="nama_obat" value="{{$drug->nama_obat}}" required>
                    @error('nama_obat')
