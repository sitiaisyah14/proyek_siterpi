...
@section('content')
    ...
    <div class="card">
        <div class="card-body">
            <h5 class="card-title">Edit Data Pakan</h5>
            <!-- Multi Columns Form -->
            <form class="row g-3" method="POST" action="{{ route('feed.update',$feed->id)}}">
                @method('PUT')
                @csrf
                <div class="col-4">
                    <label for="inputAddress" class="form-label">Nama Pakan</label>
                    <input type="text" class="form-control" name="nama_pakan" id="nama_pakan" value="{{$feed->nama_pakan}}" required>
                    @error('nama_pakan')
